...
| MCP (`cogitave-docs://`) | **served, not emitted.** MCP is a live JSON-RPC service, so it cannot be a static file. `tools/mcp/server.mjs` implements it over stdio and Streamable HTTP against spec `2025-11-25`, reading the `_api/` projection so it cannot drift from the site. Tools: `docs_search`, `docs_fetch`, `code_sample_search`, `list_catalogue`, `get_related`; resources at `cogitave-docs://learn/{uid}`. `get_related` shares its scorer with the on-page "Related" section. Retrieval is **lexical, not hybrid** - there is no embedding store. Deploying it behind `learn.cogitave.com/mcp` is a human, gated step. See [`tools/mcp/README.md`](../tools/mcp/README.md) for the full delta against the Core contract. |
